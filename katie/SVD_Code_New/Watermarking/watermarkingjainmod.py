...
# -*- coding: utf-8 -*-
"""
Created on Sat Jul 25 14:56:11 2020
...
def watermarkimage_jainmod(M,a,W):
    U, S, VT = np.linalg.svd(M, full_matrices=False)
    S = np.diag(S) 
    W = padimage(W,M)
    u_w, s_w, vt_w = np.linalg.svd(W,full_matrices=False)
    s_w = np.diag(s_w)
    watermark = (u_w @ s_w @ VT)
    watermark = padimage(watermark,M)
    A_W = M + (a * (watermark))
    return A_W

def watermarkimage_jainmod_vt_w(M,W):
    W = padimage(W,M)
    u_w, s_w, vt_w = np.linalg.svd(W,full_matrices=False)
    return vt_w

def watermark_jainmod(M, a, W,random):
    ...
    vt_w = watermarkimage_jainmod_vt_w(M_stacked,W_stacked)
    #showing the image
    ...
    plt.title("Image with watermark (Jain Mod): a = {0}".format(a))
...
def watermarkonly_jainmod(M, a, W):
    #formatting
    r_M, c_M = M.shape[:2]
    M_stacked = M.reshape(-1,c_M)
    M_type = M.dtype
...
def extractwatermark_jainmod(A_W,M,vt_w,a):
    A1 = A_W - M
...
def perceptibility_jainmod(M,a,W): #returns difference between watermarked image and original
    #formatting
    r_M, c_M = M.shape[:2]
    M_stacked = M.reshape(-1,c_M)
    M_type = M.dtype
...
def extractedwatermark_jainmod(M, a, W,random):
    #enter random=1 if using a random matrix as the watermark, else random=0 if it's an image watermark
    #formatting
    r_M, c_M = M.shape[:2]
    M_stacked = M.reshape(-1,c_M)
    ...
    watermarkedimagestacked = watermarkimage_jainmod(M_stacked, a, W_stacked)
    vt_w = watermarkimage_jainmod_vt_w(M_stacked,W_stacked)
    extractedwatermark = extractwatermark_jainmod(watermarkedimagestacked,M_stacked,vt_w,a)
...
def extractedwatermarkdifference_jainmod(M, a, W):
    #formatting
    r_M, c_M = M.shape[:2]
    M_stacked = M.reshape(-1,c_M)
    M_type = M.dtype
...
def extractedwatermarkdifferencecropped_jainmod(M, a, W,cropped,showimage,random):
    ...
    extractedwatermarkcropped = extractwatermark_jainmod(cropped_padded,M_stacked,vt_w,a)
...
def lowrankwatermarkedimage_jainmod(M, a, W,k):
    #formatting
    r_M, c_M = M.shape[:2]
    M_stacked = M.reshape(-1,c_M)
    M_type = M.dtype
...
def lowranknormdifference_jainmod(M, a, W,k,showimage,random):
    #if showimage=1, it will return the watermark extracted from the low rank image. otherwise, it will return the error.
    #if random=1, you're using a random matrix. If using an image matrix, enter random=0.
    #formatting
    r_M, c_M = M.shape[:2]
    ...
    extractedwatermark = extractwatermark_jainmod(lowrankwatermarkedimage,M_stacked,vt_w,a)
    extractedwatermark = reversepad(extractedwatermark,W_stacked)
    if showimage == 1:
        if random==0:
            extractedwatermark = extractedwatermark.reshape(r_W,c_W,-1)
...
def differentscalingfactorplots_jainmod(M,W,random):
    fig, axs = plt.subplots(4,2,figsize = (7,10))
    #scaling factor 0.05
    M005= watermarkonly_jainmod(M, 0.05,W)
    axs[0,0].imshow(M005)
    axs[0,0].set_title("Watermarked Image: a = 0.05")
    W005 = extractedwatermark_jainmod(M, 0.05,W,random)
    if random == 1:
    ...
    #scaling factor 0.5
    M050 = watermarkonly_jainmod(M, 0.5,W)
    axs[1,0].imshow(M050)
    axs[1,0].set_title("Watermarked Image: a = 0.5")
    W050 = extractedwatermark_jainmod(M, 0.5,W,random)
    if random == 1:
    ...
    #scaling factor 0.75
    M150 = watermarkonly_jainmod(M, 1.5,W)
    axs[2,0].imshow(M150)
    axs[2,0].set_title("Watermarked Image: a = 1.5")
    W150 = extractedwatermark_jainmod(M, 1.5,W,random)
    if random == 1:
    ...
    W200 = extractedwatermark_jainmod(M, 2,W,random)
    if random == 1:
...
def extractedwatermarkdifferencecropped_jainmod(M, a, W,cropped,showimage,random):
    ...
    extractedwatermarkcropped = extractwatermark_jainmod(cropped_padded,M_stacked,vt_w,a,shape)
    extractedwatermarkcropped = reversepad(extractedwatermarkcropped,W_stacked)
    #what to return
    if showimage==1:
        if random == 0:
...
def cropplots_jainmod(M,a,W,random):
    ...
    M_right_extracted = extractedwatermarkdifferencecropped_jainmod(M, a, W,M_right,random,showimage=1)
    ...
    M_bottom_extracted = extractedwatermarkdifferencecropped_jainmod(M, a, W,M_bottom,random,showimage=1)
    ...
    M_top_extracted = extractedwatermarkdifferencecropped_jainmod(M, a, W,M_top,random,showimage=1)
...
def differentalphaslowrank_jain(M,W,random):
    ...
    differences0 = []
    for rank in ranks:
        difference0 = lowrankwatermarknormdifference_jainmod(M, differentalpha[0],W,rank,0,random)
        differences0.append(difference0)
    differences1 = []
    for rank in ranks:
        difference1 = lowrankwatermarknormdifference_jainmod(M, differentalpha[1],W,rank,0,random)
        differences1.append(difference1)
    differences2 = []
    for rank in ranks:
        difference2 = lowrankwatermarknormdifference_jainmod(M, differentalpha[2],W,rank,0,random)
        differences2.append(difference2)
    differences3 = []
    for rank in ranks:
        difference3 = lowrankwatermarknormdifference_jainmod(M, differentalpha[3],W,rank,0,random)
        differences3.append(difference3)
    differences4 = []
    for rank in ranks:
        difference4 = lowrankwatermarknormdifference_jainmod(M, differentalpha[4],W,rank,0,random)
        differences4.append(difference4)
    differences5 = []
    for rank in ranks:
        difference5 = lowrankwatermarknormdifference_jainmod(M, differentalpha[5],W,rank,0,random)
        differences5.append(difference5)
    differences6 = []
    for rank in ranks:
        difference6 = lowrankwatermarknormdifference_jainmod(M, differentalpha[6],W,rank,0,random)
    ...
    if random == 1:
        plt.title('Watermark Extraction Error for Low Rank Approximations (Jain Mod) \n Random Matrix')
    else:
        plt.title('Watermark Extraction Error for Low Rank Approximations (Jain Mod)')
    plt.legend()
    plt.show()
    
def lowrankplots_jainmod(M,a,W,random):
    #if random=1, you're using a random matrix. If using an image matrix, enter random=0.
    fig, axs = plt.subplots(4,2,figsize = (7,10))
    #rank 1
    M_rank1 = lowrankwatermarkedimage_jainmod(M, a, W,1)
    axs[0,0].imshow(M_rank1)
    axs[0,0].set_title("Watermarked Image: Rank 1")
    W_rank1 = lowranknormdifference_jainmod(M, a, W,1,1,random)
    if random == 1:
    ...
    axs[0,1].set_title("Extracted Watermark")
    #rank 10
    M_rank10 = lowrankwatermarkedimage_jainmod(M, a, W,10)
    axs[1,0].imshow(M_rank10)
    axs[1,0].set_title("Watermarked Image: Rank 10")
    W_rank10 = lowranknormdifference_jainmod(M, a, W,10,1,random)
    if random == 1:
    ...
    axs[1,1].set_title("Extracted Watermark")
    #rank 100
    M_rank100 = lowrankwatermarkedimage_jainmod(M, a, W,100)
    axs[2,0].imshow(M_rank100)
    axs[2,0].set_title("Watermarked Image: Rank 100")
    W_rank100 = lowranknormdifference_jainmod(M, a, W,100,1,random)
    if random == 1:
    ...
    axs[2,1].set_title("Extracted Watermark")
    #rank 400
    M_rank400 = lowrankwatermarkedimage_jainmod(M, a, W,400)
    axs[3,0].imshow(M_rank400)
    axs[3,0].set_title("Watermarked Image: Rank 400")
    W_rank400 = lowranknormdifference_jainmod(M, a, W,400,1,random)
    if random == 1:
